...
export default function DashboardGuru() {
  ...
  const handleSaveQuestion = async (e) => {
    ...
    if (editForm.story_json && !validateJSON(editForm.story_json)) {
      setValidationError('❌ story_json tidak valid! Periksa format JSON.');
      return;
    }
    if (editForm.options_json && !validateJSON(editForm.options_json)) {
      setValidationError('❌ options_json tidak valid! Periksa format JSON.');
      return;
    }
    if (editForm.correct_config && !validateJSON(editForm.correct_config)) {
      setValidationError('❌ correct_config tidak valid! Periksa format JSON.');
      return;
    }
  ...
  const handleCreateQuestion = async (e) => {
    ...
    if (!createForm.level_number || !createForm.question_text || !createForm.options_json || !createForm.correct_config) {
      setValidationError('❌ Level, pertanyaan, opsi, dan kunci jawaban wajib diisi!');
      return;
    }

    // Validasi JSON fields
    if (createForm.story_json && !validateJSON(createForm.story_json)) {
      setValidationError('❌ story_json tidak valid! Periksa format JSON.');
      return;
    }
    if (!validateJSON(createForm.options_json)) {
      setValidationError('❌ options_json tidak valid! Periksa format JSON.');
      return;
    }
    if (!validateJSON(createForm.correct_config)) {
      setValidationError('❌ correct_config tidak valid! Periksa format JSON.');
      return;
    }
    ...
    const token = localStorage.getItem('adminToken');
    try {
      await axios.post(`${import.meta.env.VITE_API_URL}/api/admin/questions`, createForm, {
        headers: { Authorization: `Bearer ${token}` }
      });
            ...
            <motion.div 
              ...
              <form onSubmit={handleSaveQuestion} className="flex-1 overflow-y-auto p-6 md:p-8 space-y-6 city-bg relative">
                ...
                <div className="glass p-6 rounded-2xl border border-emerald-100">
                  ...
                  <p className="text-xs text-stone-500 mb-4 font-bold">Contoh: <code>[{`{"character":"Tutor", "dialog":"Halo!"}`}]</code>. Pastikan menggunakan format Array of Objects JSON yang valid.</p>
                  <textarea 
              ...
              <form onSubmit={handleCreateQuestion} className="flex-1 overflow-y-auto p-6 md:p-8 space-y-6 city-bg relative">
                ...
                <div className="glass p-6 rounded-2xl">
                  ...
                  <textarea 
                    ...
                    className="w-full px-4 py-3 rounded-xl border-2 border-transparent bg-white shadow-sm focus:border-blue-400 outline-none transition-all resize-y"
                    rows="2"
                    placeholder="Opsional: Pesan yang ditampilkan saat siswa menjawab benar" />
                </div>

                <div className="glass p-6 rounded-2xl border border-emerald-100">
                  <label className="block text-sm font-black text-emerald-800 mb-1">Cerita Naratif (Visual Novel - story_json)</label>
                  <p className="text-xs text-stone-500 mb-4 font-bold">Contoh: <code>[{`{"character":"Tutor", "dialog":"Halo!"}`}]</code></p>
                  <textarea 
                    value={createForm.story_json}
                    onChange={(e) => setCreateForm({...createForm, story_json: e.target.value})}
                    className="w-full px-4 py-3 rounded-xl border-2 border-transparent bg-emerald-50 text-emerald-900 shadow-sm focus:border-emerald-400 outline-none transition-all font-mono text-sm leading-relaxed"
                    rows="5"
                    placeholder="[] (atau kosongkan jika tidak perlu cerita)" />
                </div>
                
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="glass p-6 rounded-2xl border border-blue-100">
                    <label className="block text-sm font-black text-blue-800 mb-2">Opsi (options_json) *</label>
                    <textarea 
                      required
                      value={createForm.options_json}
                      onChange={(e) => setCreateForm({...createForm, options_json: e.target.value})}
                      className="w-full px-4 py-3 rounded-xl border-2 border-transparent bg-blue-50 shadow-sm focus:border-blue-400 outline-none transition-all font-mono text-sm"
                      rows="4"
                      placeholder='[{"id": "1", "label": "Pilihan A"}]' />
                  </div>
                  <div className="glass p-6 rounded-2xl border border-blue-100">
                    <label className="block text-sm font-black text-blue-800 mb-2">Kunci / Target (correct_config) *</label>
                    <textarea 
                      required
                      value={createForm.correct_config}
                      onChange={(e) => setCreateForm({...createForm, correct_config: e.target.value})}
                      className="w-full px-4 py-3 rounded-xl border-2 border-transparent bg-blue-50 shadow-sm focus:border-blue-400 outline-none transition-all font-mono text-sm"
                      rows="4"
                      placeholder='{"correct": ["1"]}' />
                  </div>
                </div>
